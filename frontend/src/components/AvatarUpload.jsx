...
export default function AvatarUpload({ currentAvatar, onUploadSuccess }) {
  ...
  const handleFileChange = (e) => {
    ...
    if (!file.type.startsWith("image/")) {
      setError(t("avatar.select_image", "Please select an image file."));
      return;
    }

    if (file.size > 2 * 1024 * 1024) {
      setError(t("avatar.max_size", "Image must be smaller than 2MB."));
      return;
    }
  ...
  const uploadFile = async (file) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || t("avatar.upload_failed", "Upload failed. Please try again."));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex flex-col items-center gap-4">
      <div className="relative group">
        ...
        <button
          ...
          aria-label={t("avatar.upload_photo", "Upload photo")}
          disabled={loading}
        >
      ...
      <p className="text-[10px] uppercase tracking-widest text-muted font-bold">
        {loading ? t("avatar.uploading", "Uploading...") : t("avatar.hint", "JPG, PNG or WEBP (Max 2MB)")}
      </p>
    </div>
  );
}
